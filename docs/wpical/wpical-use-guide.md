...
The first step in using Wpical is calibrating your camera. In order to do this, a video capture is needed. It is better to get videos through a phone instead of your laptop camera. These videos will be used for the calibrations.

Open Wpical, go to the **Camera Calibration** tab, and use/create a video which shows the **5x5 ChArUco board** at different angles and distances (keep it close). Run the calibration. Wpical outputs a camera calibration JSON which you'll use in the next step.

(Pick **MRcal** over OpenCV)
...
Before running the optimization, **pin one tag**
